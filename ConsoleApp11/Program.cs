...
namespace ns
{
    class tClass
    {
        ...
        public static void OnInitialize(ref List<Cheque> mainData)
        {
            using (var reader = new StreamReader("cheque.json"))
            {
                var tmp = reader.ReadToEnd();
                mainData = JsonConvert.DeserializeObject<List<Cheque>>(tmp);
            }
        }

        public static void BeforeExit(List<Cheque> mainData)
        {
            using (var writer = new StreamWriter("cheque.json"))
            {
